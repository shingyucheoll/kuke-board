...
class CommentServiceTest {
    ...
    @Test
    @DisplayName("삭제할 댓글이 자식이 있을 경우 삭제 표시만 합니다.")
    ...
    @Test
    @DisplayName("하위 댓글이 삭제되고, 삭제되지 않은 부모일경우, 하위 댓글만 삭제한다.")
    void deleteShouldDeleteChildOnlyIfNotDeletedParent() {
        ...
        // never 는 호출되지 않았음을 검증합니다.
        verify(commentRepository, never()).delete(parentComment);
    }

    @Test
    @DisplayName("하위 댓글이 삭제되고, 삭제된 부모일 경우, 재귀적으로 모두 삭제한다.")
